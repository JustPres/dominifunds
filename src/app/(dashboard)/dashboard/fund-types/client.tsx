...
export default function FundTypesClient() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["fund-types", orgId],
    queryFn: () => getFundTypes(orgId as string, true),
    enabled: !!orgId,
  });
    ...
    <div className="flex h-full flex-col pb-8 font-body">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-[#625f5f]">
          Manage the active and archived fund definitions that power collections, installments, and reporting.
        </p>
      </div>
        ...
        {isLoading ? (
          ...
          <>
            <div className="flex items-center gap-4 rounded-2xl border border-blue-200 bg-blue-50 p-5 shadow-sm">
              ...
              <div>
                <p className="text-xs font-bold uppercase tracking-wide text-blue-800/70">Active Categories</p>
            ...
            <div className="flex items-center gap-4 rounded-2xl border border-red-200 bg-red-50 p-5 shadow-sm">
              ...
              <div>
                ...
                <h3 className="font-display text-3xl font-bold text-red-700">
                  {currencyFormatter.format(data?.kpis.requiredPerSemesterTotal || 0)}
                </h3>
              </div>
            </div>
