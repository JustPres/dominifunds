...
export default function InstallmentsClient() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["installments", orgId],
    queryFn: () => getInstallmentData(orgId as string),
    enabled: !!orgId,
  });
    ...
    <div className="flex h-full flex-col pb-8 font-body">
      <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-[#625f5f]">
            Monitor scheduled installment plans, overdue entries, and completion progress for your organization.
          </p>
        </div>

        <div className="flex shrink-0 items-center justify-end">
          <AddInstallmentDialog orgId={orgId || ""} />
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="flex items-center gap-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-5 shadow-sm">
              ...
              <div>
                <p className="text-xs font-bold uppercase tracking-wide text-emerald-800/70">Completed Plans</p>
                <h3 className="font-display text-3xl font-bold text-emerald-700">
                  {data?.kpis.completedPlansSemester}
                </h3>
              </div>
            </div>
          </>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
          {[...Array(3)].map((_, index) => (
            <Skeleton key={index} className="h-[350px] w-full rounded-2xl" />
          ))}
        </div>
      ) : data?.plans.length === 0 ? (
        <div className="flex min-h-[400px] flex-col items-center justify-center rounded-2xl border border-dashed border-[#F0ECEC] bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-[#625f5f]">Create the first plan to start tracking split payments.</p>
          <div className="mt-6">
            <AddInstallmentDialog orgId={orgId || ""} />
          </div>
        </div>
