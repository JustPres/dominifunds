...
export async function GET(
  ...
  return NextResponse.json({
    logs: logs.map((log) => ({
      ...
      note: log.note,
      actorName: log.actor?.name ?? log.actorNameSnapshot ?? "System",
      actorEmail: log.actor?.email ?? log.actorEmailSnapshot ?? null,
      actorOfficerRole: log.actorOfficerRoleSnapshot ?? log.actorOfficerRole,
