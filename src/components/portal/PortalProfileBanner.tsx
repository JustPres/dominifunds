...
export function PortalProfileBanner({ session, kpis }: { session: Session | null; kpis: KPIProps }) {
  ...
  const yearLevel = formatYearLevelLabel(session?.user?.yearLevel, "Student");

  return (
    <div className="mb-8 flex flex-col gap-6">
      <div className="relative overflow-hidden rounded-[12px] border border-[#DDD8D6] bg-white p-8 shadow-sm">
        <div className="relative z-10 flex flex-col items-start gap-6 sm:flex-row sm:items-center">
          <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl border border-[#F0ECEC] bg-[#F9F7F6] shadow-sm">
          ...
          <div className="flex-1 text-[#343434]">
            <div className="flex items-center gap-3">
              ...
              <span className="inline-flex items-center rounded-full border border-emerald-100 bg-emerald-50 px-2.5 py-0.5 text-[11px] font-bold tracking-widest text-emerald-700">
                GOOD STANDING
              </span>
            </div>
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        <div className="flex flex-col gap-1 rounded-[12px] border border-[#DDD8D6] bg-white p-5 shadow-sm">
          ...
          <h3 className="mt-2 font-display text-2xl font-bold text-[#343434]">
            {currencyFormatter.format(kpis.totalPaid)}
          </h3>
        </div>
        ...
        <div className="flex flex-col gap-1 rounded-[12px] border border-[#DDD8D6] bg-white p-5 shadow-sm">
          ...
          <h3 className="mt-2 font-display text-2xl font-bold text-[#343434]">{kpis.activeInstallmentPlans}</h3>
        </div>
        ...
        <div className="flex flex-col gap-1 rounded-[12px] border border-[#DDD8D6] bg-white p-5 shadow-sm">
          ...
          <h3 className="mt-2 font-display text-2xl font-bold text-[#a12124]">
            {currencyFormatter.format(kpis.nextPaymentDueAmount ?? 0)}
          </h3>
        </div>
        ...
        <div className="flex flex-col gap-1 rounded-[12px] border border-[#DDD8D6] bg-white p-5 shadow-sm">
          ...
          <h3 className="mt-auto font-display text-lg font-bold text-[#343434]">
