...
export function PortalInstallmentsBlock({ plans }: { plans: PortalInstallmentPlan[] }) {
  if (plans.length === 0) {
    return (
      <div className="flex h-40 flex-col items-center justify-center rounded-2xl border border-[#F0ECEC] bg-white p-6 shadow-sm">
  ...
  return (
    <div className="flex flex-col gap-4">
      {plans.map((plan) => (
        <div key={plan.id} className="overflow-hidden rounded-2xl border border-[#F0ECEC] bg-white shadow-sm">
          {/* Header */}
          <div className="border-b border-[#F0ECEC] bg-[#F9F7F6] p-5">
...
export function PortalObligationsGrid({ obligations }: { obligations: PortalObligation[] }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      {obligations.map((ob) => (
        <div key={ob.id} className="flex items-center justify-between rounded-xl border border-[#F0ECEC] bg-white p-4 shadow-sm transition-all hover:border-[#a12124]/30 hover:shadow-md">
